...
void step_ucode_Translate(Linklst **list, Mainmem **mlist, LinkedCpuStk **clist, LinkedLblStk **lblist, Qlst **qlist, char *file_name){
	Linklst *root = *list;
	Mainmem *mroot = *mlist;
	char func_label[20] = "", label[20], return_label[20], ch;
	int addr = 0, data, tf_check = -1, push_check = 0;
	int stk_top, stk_top_under, insert_read, step_number = 1, c;
	...
	while (root != NULL){
		if (root->opcode != 0){
			fout << step_number << ". 단계 명령어: " << root->data << endl;
			cout << step_number++ << ". 단계 명령어: " << root->data << endl;
		}
		else{
			fout << step_number << ". 레이블: " << root->data << endl;
			cout << step_number++ << ". 레이블: " << root->data << endl;
		}
		if (root->opcode == 1){//bgn
			...
			if (size == 1){
				...
				cout << addr - 1 << "까지 설정이 됨" << endl;
			}
			fout << " - 블럭: " << root->operand1 << endl;
			fout << " - 오프셋: " << root->operand2 << endl;
			fout << " - 사이즈: " << root->operand3 << endl;
			cout << " - 블럭: " << root->operand1 << endl;
			cout << " - 오프셋: " << root->operand2 << endl;
			cout << " - 사이즈: " << root->operand3 << endl;
		}
		else if (root->opcode == 3){//nop
			nop_num++;
			print_Memory(mlist, &fout, 1);
			print_CpuStack(clist, &fout, 1);
			cout << ">> 다음 단계로 진행하려면 엔터를 누르세요.\n";
			cout << ">> 한번에 진행하려면 1을 누르세요.\n";
			fout << "--------------------------------------------\n";
			cout << "--------------------------------------------\n";
			while (1){
				ch = _getch();
				if (ch == 13){
					...
					root = root->rlink;
					fout.close();
					ucode_Translate(list, mlist, clist, lblist, qlist, file_name);
					return;
				}
				while ((c = getchar()) != '\n'&&c != EOF);
			}
			...
			cout << " - 지역 변수 갯수: " << root->operand1 << endl;
			int i;
			if ((*qlist)->front != NULL){
			...
			cout << " - return될 Label: " << func_label << endl;
		}
			...
			if (strcmp(root->operand1, "write") == 0){
				...
				cout << " - write 함수 호출" << endl;
				cout << " - 출력: " << pop_data << " " << endl;
			}
			else if (strcmp(root->operand1, "read") == 0){
				cout << " - read 함수 호출" << endl;
				cout << " - 값 입력: ";
				cin >> insert_read;
			...
			cout << " - 함수: " << func_label << endl;
			cout << " - 블록: " << root->operand1 << endl;
			cout << " - 오프셋: " << root->operand2 << endl;
			cout << " - 가져온 값: " << data << endl;
		}
			...
			cout << " - 메모리에 넣을 값:  " << stk_top << endl;
			cout << " - 메모리 주소: " << stk_top_under << endl;
		}
		else if (root->opcode == 16){//not
			not_num++;
			stk_top = cpustk_Pop(clist);
			fout << " - 바뀌기 전 값:  " << stk_top << endl;
			cout << " - 바뀌기 전 값:  " << stk_top << endl;
			cpustk_Push(clist, !stk_top);
			fout << " - 진리 값: " << !stk_top << endl;
			cout << " - 진리 값: " << !stk_top << endl;
		}
			...
			cout << " - 바뀌기 전의 두 값: " << stk_top_under << " " << stk_top << endl;
			cpustk_Push(clist, stk_top);
			...
			cout << " - 건너 뛸 Label: " << root->operand1 << endl;
